...
public class Agencia {
    ...
    /*
     * Dado un videojuego añade una puntuación
     */
    public void ponerPuntuacionVideojuego(int codigoVideojuego, int puntuacion){
        videojuegosRegistrados.get(codigoVideojuego).anyadirPuntuacion(puntuacion);
    }
    
    /*
     * Dado un gmail y una contraseña comprueba si existe el usuario y si existe devuelve su DNI
     */
    public String comprobarUsuario(String correo, String contrasenia){
        String cadena = null;        
        for (Map.Entry<String, Usuario> usuarioAux : usuariosRegistrados.entrySet()) {
            if((usuarioAux.getValue().getCorreoElectronico().equals(correo)) && 
               (usuarioAux.getValue().getContrasenia().equals(contrasenia)) ){
                    cadena = usuarioAux.getKey();
            } 
        }      
        return cadena;
    }
